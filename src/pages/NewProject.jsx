...
// Usa traduzirErroIA do módulo partilhado (generateSection.js)
...
function AIBadge({ onRegenerate, loading }) {
  return (
    <div className="flex items-center justify-between mt-2">
      <span className="inline-flex items-center gap-1.5 text-xs font-medium text-blue-600 bg-primary-400/10 px-2.5 py-1 rounded-full border border-primary-400/20">
      ...
      <button
        ...
        disabled={loading}
        className="inline-flex items-center gap-1.5 text-xs text-slate-500 hover:text-slate-900 transition-colors disabled:opacity-40"
      >
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <button
                  ...
                  className={`relative flex flex-col items-start gap-2 rounded-xl p-4 border-2 transition-all text-left ${
                    form.projectType === 'tcc'
                      ? 'border-primary-500 bg-blue-50 text-slate-900'
                      : 'border-slate-200 bg-slate-50 text-slate-500 hover:border-slate-200'
                  }`}
                >
                  <div className="flex items-center justify-between w-full">
                    <div className="flex items-center gap-2">
                      <GraduationCap className={`w-5 h-5 ${form.projectType === 'tcc' ? 'text-blue-600' : 'text-slate-400'}`} />
                  ...
                  {form.projectType === 'tcc' && (
                    <CheckCircle2 className="absolute bottom-3 right-3 w-4 h-4 text-blue-600" />
                  )}
                </button>
                ...
                <button
                  ...
                  className={`relative flex flex-col items-start gap-2 rounded-xl p-4 border-2 transition-all text-left ${
                    form.projectType === 'anteprojecto'
                      ? 'border-accent-500 bg-indigo-50 text-slate-900'
                      : 'border-slate-200 bg-slate-50 text-slate-500 hover:border-slate-200'
                  }`}
                >
                  <div className="flex items-center justify-between w-full">
                    <div className="flex items-center gap-2">
                      <FileText className={`w-5 h-5 ${form.projectType === 'anteprojecto' ? 'text-indigo-600' : 'text-slate-400'}`} />
                  ...
                  {form.projectType === 'anteprojecto' && (
                    <CheckCircle2 className="absolute bottom-3 right-3 w-4 h-4 text-indigo-600" />
                  )}
                </button>
              </div>
            </div>
            ...
            <div className="glass-card rounded-2xl p-6 space-y-5">
              ...
              <div>
                <label className="flex items-center gap-1.5 text-sm font-medium text-slate-500 mb-1.5">
                  ...
                  <span className="text-xs text-slate-400 font-normal ml-1">(opcional — melhora as sugestões da IA)</span>
                </label>
              ...
              <div ref={titleWrapperRef}>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 text-xs font-medium text-indigo-500 hover:text-accent-200 bg-indigo-50 hover:bg-indigo-600/20 border border-accent-500/20 px-2.5 py-1 rounded-full transition-all disabled:opacity-40"
                  ...
                  <AnimatePresence>
                    {showSuggestions && (loadingSuggestions || suggestions.length > 0) && (
                      <motion.div
                        ...
                        className="absolute left-0 right-0 top-full mt-1 z-50 rounded-xl border border-accent-500/25 bg-slate-50/95 backdrop-blur-sm shadow-2xl overflow-hidden"
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between px-3 py-2 border-b border-slate-200">
                          ...
                          <button
                            ...
                            className="text-slate-400 hover:text-slate-500 p-0.5 rounded transition-colors"
                          >
                            <X className="w-3.5 h-3.5" />
                          </button>
                        </div>
                        ...
                        {loadingSuggestions && (
                          <div className="p-2 space-y-1.5">
                            {[85, 70, 90, 65, 80].map((w, i) => (
                              <div
                                key={i}
                                className="h-8 rounded-lg bg-slate-50 animate-pulse"
                                style={{ width: `${w}%` }}
                              />
                            ))}
                          </div>
                          ...
                          <div className="py-1">
                            {suggestions.map((s, i) => (
                              <motion.button
                                ...
                                className="w-full text-left px-3 py-2.5 text-sm text-slate-700 hover:text-slate-900 hover:bg-indigo-50 transition-colors flex items-center gap-2.5 group"
                              >
                                <ChevronDown className="w-3.5 h-3.5 text-indigo-600 flex-shrink-0 -rotate-90 opacity-0 group-hover:opacity-100 transition-opacity" />
                            ...
                            <div className="border-t border-slate-200 px-3 py-2">
                              <button
                                ...
                                className="text-xs text-slate-400 hover:text-indigo-500 flex items-center gap-1.5 transition-colors"
                              >
                ...
                <p className="text-xs text-slate-400 mt-1.5">
                  A IA sugere títulos automaticamente enquanto escreve. Use <strong className="text-indigo-600">Inspirar-me</strong> para ideias do zero. Ao sair do campo, o tema e o problema são gerados.
                </p>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  {generating && (
                    <span className="inline-flex items-center gap-1.5 text-xs text-blue-600 animate-pulse">
            ...
            <div className="glass-card rounded-2xl p-6 space-y-5">
              ...
              <div className="grid sm:grid-cols-2 gap-5">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-1">Escreva ou seleccione da lista de sugestões.</p>
                </div>
            ...
            <div className="glass-light rounded-xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-slate-500">
                <strong className="text-slate-700">Dica:</strong> A IA gera o Tema e o Problema de
